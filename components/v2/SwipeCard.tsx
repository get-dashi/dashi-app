...
export function V2SwipeCard({ venue, position, style, onRef }: SwipeCardProps) {
  ...
  return (
    <div
      ref={onRef}
      className="absolute w-full select-none touch-none cursor-grab"
      style={{
    ...
    >
      {/* Swipe stamps */}
      <div
        className="like-stamp absolute top-10 left-5 z-20 pointer-events-none"
        style={{
      ...
      <div
        className="nope-stamp absolute top-10 right-5 z-20 pointer-events-none"
        style={{
          border: '2.5px solid #FF375F',
          color: '#FF375F',
          fontSize: '1rem',
          fontWeight: 900,
          letterSpacing: '0.1em',
      ...
      <div className="absolute top-4 left-4 z-10 flex flex-col gap-1.5">
        {venue.featured && !michelinTier && (
          <span
            className="text-[0.52rem] font-black tracking-[0.1em] uppercase px-2.5 py-1.5 rounded-[8px]"
        ...
        {venue.glutenFree && (
          <span
            className="text-[0.52rem] font-black tracking-[0.08em] uppercase px-2.5 py-1.5 rounded-[8px]"
      ...
      <div className="absolute top-4 right-4 z-10 flex flex-col items-end gap-1.5">
        {michelinTier && (
          <div
            ...
            style={{
              background: michelinTier === 'star' ? 'rgba(127,29,29,0.6)' : michelinTier === 'bib' ? 'rgba(120,53,15,0.6)' : 'rgba(30,30,40,0.7)',
              border: `1px solid ${michelinTier === 'star' ? 'rgba(239,68,68,0.5)' : michelinTier === 'bib' ? 'rgba(249,115,22,0.4)' : 'rgba(255,255,255,0.2)'}`,
              backdropFilter: 'blur(12px)',
            }}
          >
            <span style={{
              fontSize: michelinTier === 'star' ? '0.85rem' : '0.7rem',
              color: michelinTier === 'star' ? '#f87171' : michelinTier === 'bib' ? '#fb923c' : 'rgba(255,255,255,0.6)',
            }}>
            ...
            <span style={{
              ...
              color: michelinTier === 'star' ? '#fca5a5' : michelinTier === 'bib' ? '#fdba74' : 'rgba(255,255,255,0.55)',
            }}>
      ...
      <div style={{ position: 'relative', width: '100%', height: 440 }}>
        ...
        <div style={{
          position: 'absolute',
          inset: 0,
          background: 'linear-gradient(to top, rgba(0,0,0,0.97) 0%, rgba(0,0,0,0.6) 45%, rgba(0,0,0,0.15) 75%, transparent 100%)',
        }} />
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: '20px 20px 20px' }}>
          ...
          {/* Name */}
          <h2 style={{ fontSize: '1.45rem', fontWeight: 900, letterSpacing: '-0.025em', lineHeight: 1.1, marginBottom: 6 }}>
            {venue.name}
          </h2>
          ...
          {/* Curator note */}
          {venue.curatorNote && (
            <p style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.6)', lineHeight: 1.55, marginBottom: 12 }} className="line-clamp-2">
              <span style={{ fontWeight: 700, color: 'rgba(255,255,255,0.75)' }}>Ricky: </span>
              {venue.curatorNote}
            </p>
          )}
      ...
      {/* Floating heart button (top-right of card, over image) */}
      <button
        className="absolute z-20 transition-all active:scale-90"
        style={{
          bottom: venue.bookingPlatform ? 80 : 20,
          right: 20,
          width: 44,
          height: 44,
          borderRadius: '50%',
